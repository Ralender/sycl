...
public:
  ...
  pi_uint64 get_start_time() const {
    assert(start_time && "has not started");
    return start_time;
  }
  pi_uint64 get_submit_time() const {
    assert(start_time && "has been submitted");
    return submit_time;
  }
  pi_uint64 get_completed_time() const {
    assert(start_time && "has been completed");
    ...
    if (properties & PI_QUEUE_FLAG_PROFILING_ENABLE)
      std::cerr
          << "warning: support for profiling is only partial. pi_xrt is fully "
             "synchonous so it will detect when an event is completed only "
             "when the user waits on the event. if the user waits on the event "
             "just after starting it, the profiling information is reliable. "
             "but if the user uses wait on an event that was already finished "
